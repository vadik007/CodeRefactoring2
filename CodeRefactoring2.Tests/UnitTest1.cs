...
namespace CodeRefactoring2.Tests
{
    ...
    {
        const string tempSavePath = @".\save.xml";
        [Test]
...
I recommend you launch the debugger in the menu to the left and analyze the data to find out the cause.
        ...
        private void PrintSrcFile(SourceFileHasher.NewSourceEntry entry)
        {
            
        }

        private void PrintSrcFile(string path, int line, string logLine)
        {
            try
            {
                var theLine = File.ReadAllLines(path)[line];

                Console.ForegroundColor = ConsoleColor.Red; Console.BackgroundColor = ConsoleColor.DarkMagenta;
                Console.WriteLine("{0,30} - ", logLine);
                Console.WriteLine("{0,30}", theLine);
            }
            catch (Exception e)
            {
                Console.WriteLine(e);
